...
def replace_Ellipsis(df, var):
    """
    Iterate through index of given var
    Replace Ellipsis in each row of var with np.nan
    """
    new_column = []
    for array in df[var]:
        new_array = np.where(array==Ellipsis, np.nan, array)
